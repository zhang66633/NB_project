<template>
  <div class="space-y-4">
    <div>
      <h3 class="text-base font-semibold">论文写作智能体</h3>
      <p class="text-sm text-muted-foreground mt-1">负责生成规范论文</p>
    </div>
...
onMounted(() => {
  const saved = store.getConfig(AgentType.WRITING);
  if (saved) {
...
function save() {
  store.setConfig(AgentType.WRITING, { ...config });
}
</script>
