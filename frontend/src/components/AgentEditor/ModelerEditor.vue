<template>
  <div class="space-y-4">
    <div>
      <h3 class="text-base font-semibold">模型构建智能体</h3>
      <p class="text-sm text-muted-foreground mt-1">负责选择或设计数学模型</p>
    </div>
...
onMounted(() => {
  const saved = store.getConfig(AgentType.MODELING);
  if (saved) {
...
function save() {
  store.setConfig(AgentType.MODELING, { ...config });
}
</script>
