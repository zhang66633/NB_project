<template>
  <div class="space-y-4">
    <div>
      <h3 class="text-base font-semibold">问题分析智能体</h3>
      <p class="text-sm text-muted-foreground mt-1">负责理解题意、提取关键信息、明确问题边界</p>
    </div>
...
onMounted(() => {
  const saved = store.getConfig(AgentType.ANALYSIS);
  if (saved) {
...
function save() {
  store.setConfig(AgentType.ANALYSIS, { ...config });
}
</script>
