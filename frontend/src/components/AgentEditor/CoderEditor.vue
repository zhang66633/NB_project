<template>
  <div class="space-y-4">
    <div>
      <h3 class="text-base font-semibold">求解计算智能体</h3>
      <p class="text-sm text-muted-foreground mt-1">负责编写求解代码、执行计算</p>
    </div>
...
onMounted(() => {
  const saved = store.getConfig(AgentType.SOLVING);
  if (saved) {
...
function save() {
  store.setConfig(AgentType.SOLVING, { ...config });
}
</script>
